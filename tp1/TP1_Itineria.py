...
def crowfliesdistance(town1, town2):
    earth_radius = 6371
    cfd = earth_radius * math.acos(
        math.sin(math.radians(town1.latitude)) * math.sin(math.radians(town2.latitude))
        + math.cos(math.radians(town1.latitude)) * math.cos(math.radians(town2.latitude))
        * math.cos(math.radians(town1.longitude) - math.radians(town2.longitude)))
    return cfd

# A-Star
def a_star(start_town, end_town):
    initial_node = Node(start_town, crowfliesdistance(start_town, end_town), None, None)
    ...
    while not frontier.empty():
        ...
        for neighbour, neighbour_road in node.town.neighbours.items():
            child = Node(neighbour, node.cost - crowfliesdistance(node.town, end_town)
                         + neighbour_road.distance + crowfliesdistance(neighbour, end_town), node, neighbour_road)
...
def greedy_search(start_town, end_town):
    ...
    while not frontier.empty():
        ...
        explored.append(node.town)
        for neighbour, neighbour_road in node.town.neighbours.items():
            child = Node(neighbour, crowfliesdistance(neighbour, end_town), node, neighbour_road)
            if child.town not in explored:
                not_in_frontier = True
                frontier_list = list(frontier.queue)
                for frontier_node in frontier_list:
